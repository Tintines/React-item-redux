...
import { Button, Icon, Card, Table, message, Modal, Form, Input, Tree} from 'antd';
...
@Form.create()      // 使用form表单时, 一定需要对组件进行包装!!!
class Role extends Component{
  state = {
    isShowAdd: false,
    isShowAuth: false,
    roleList: [],
    menuList,           // 选中的菜单, 添加的权限
    checkedKeys: [],    // 当前操作的角色id
    _id: '',
  }
  ...
  renderTreeNodes = (data) =>
      data.map(item => {
      if (item.children) {    // 数据结构中有children属性
        return (              // 递归调用, TreeNode标签里禁止写注释!!! 报错
